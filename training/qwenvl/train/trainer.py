...
def qwen2vl_lora_forward(
    ...
    **kwargs: Unpack[FlashAttentionKwargs],
    ...
    query_states = self.q_proj(hidden_states) + self.q_proj_lora(hidden_states)
    key_states = self.k_proj(hidden_states) + self.k_proj_lora(hidden_states)
    value_states = self.v_proj(hidden_states) + self.v_proj_lora(hidden_states)
    ...
    attn_output = attn_output.reshape(bsz, q_len, -1).contiguous()
    attn_output = self.o_proj(attn_output) + self.o_proj_lora(attn_output)
    return attn_output, attn_weights
